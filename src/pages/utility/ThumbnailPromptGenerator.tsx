...
import { ImageIcon } from '../../components/icons/UtilityIcons';
...
const UTILITY_SLUG = 'thumbnail-prompt-generator';
const STORAGE_KEY = `formState_${UTILITY_SLUG}`;

const ThumbnailPromptGenerator: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (savedState) {
        const { topic: savedTopic, tone: savedTone } = JSON.parse(savedState);
        setTopic(savedTopic || '');
        setTone(savedTone || '');
      }
    } catch (err) {
  ...
  useEffect(() => {
    try {
      const stateToSave = JSON.stringify({ topic, tone });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    if (!topic || !tone) {
      setError('Please fill out both video topic and tone.');
      return;
    }
    ...
    setResults(null);
    try {
      const generatedResults = await generateThumbnailPrompts(topic, tone);
      ...
      const errorMessage = err.message || 'An error occurred while generating prompts. Please try again.';
  ...
  return (
    <div className="max-w-3xl mx-auto animate-fade-in-up">
      <div className="flex items-center space-x-3 mb-6">
        ...
        <h1 className="text-3xl font-bold">Thumbnail Prompt Generator</h1>
      </div>
      <p className="mb-6 text-gray-600 dark:text-gray-400">Get 10 creative thumbnail prompt ideas for your image AI, plus color and font cues.</p>
      ...
      <form onSubmit={handleSubmit} className="p-6 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 space-y-4">
        <div>
          <label htmlFor="topic" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Video Topic</label>
          <input
            type="text"
            id="topic"
            value={topic}
        ...
        <div>
          <label htmlFor="tone" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Desired Tone</label>
          <input
            type="text"
            id="tone"
            value={tone}
            onChange={(e) => setTone(e.target.value)}
            placeholder="e.g., Dramatic, professional, minimalist"
            className="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-brand-secondary focus:border-brand-secondary"
          />
        </div>
        ...
        <div className="pt-2">
            ...
            >
              {isLoading ? 'Generating Ideas...' : 'Generate Prompts'}
            </button>
        </div>
      </form>
      ...
      {isLoading && (
        <div className="mt-8 text-center">
            ...
            <p className="mt-2 text-gray-600 dark:text-gray-400">Brainstorming with AI...</p>
        </div>
      )}
      ...
      {results && (
        <div className="mt-8 space-y-6">
            <div>
                <h2 className="text-2xl font-bold mb-4">Prompt Ideas</h2>
                <div className="space-y-3">
                    {results.prompts.map((prompt, index) => (
                    <div key={index} className="group flex justify-between items-center p-4 bg-gray-100 dark:bg-gray-800 rounded-md">
                        <p className="text-gray-800 dark:text-gray-200">{prompt}</p>
                        <button onClick={() => handleCopyToClipboard(prompt)} className="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600">
            ...
            <div>
                <h2 className="text-2xl font-bold mb-4">Color & Font Cues</h2>
                <div className="space-y-2">
                    {results.cues.map((cue, index) => (
                        <p key={index} className="p-3 bg-gray-100 dark:bg-gray-800 rounded-md">{cue}</p>
                    ))}
                </div>
            </div>
        </div>
...
export default ThumbnailPromptGenerator;