...
const SignupPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate('/chat'); // Redirect to a protected page on successful signup
    } catch (err: any) {
  ...
  return (
    <div className="max-w-md mx-auto mt-10 animate-fade-in-up">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-extrabold text-light-primary">Create Your Account</h1>
        <p className="text-light-secondary">Join Mike's AI Forge today.</p>
      </div>
      <form onSubmit={handleSubmit} className="p-8 bg-dark-secondary rounded-lg border border-border-dark space-y-6 shadow-lg">
        ...
        <div>
          <label htmlFor="confirm-password" className="block text-sm font-medium text-light-secondary">Confirm Password</label>
          <input
            type="password"
            id="confirm-password"
